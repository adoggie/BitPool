...
from elabs.service.bitpool.shrwlock import RWLock

from elabs.service.bitpool.get_bar_time import get_bar_time
from elabs.service.bitpool.basetype import Bar,get_ntime
...
class DataBlock(object):
	...
	def fill_index(self):
		...
		for n in xrange(self.owner.days):
	...
	def put_data(self,m1):
		...
		self.open_ptr[d] = bar.open
		self.high_ptr[d] = bar.high
		self.low_ptr[d]  = bar.low
		self.close_ptr[d] = bar.close
		self.opi_ptr[d] =  bar.opi
		self.vol_ptr[d] = bar.vol
...
class DataBucket(object):
	""""""
	def __init__(self,sdm,start,days,symbols,period):
		self.sdm = sdm
		self.start = start
		self.days = days
