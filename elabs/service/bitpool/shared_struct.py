...
from elabs.service.bitpool.basetype import Bar
...
from elabs.service.bitpool.data_bucket import DataBucket
...
class SharedDataManager(object):
	...
	def init(self,**kwargs):
		...
		self.bucket = DataBucket(self, start, days, symbols,self.get_period())
		multiple = 1
	...
	def put_data(self,symbol,bar):
		...
		if not isinstance(time, datetime.datetime):
			time = parse(time)
			bar.time = time
		self.bucket.put_data(symbol,bar)
	
	def sythesize(self, m1,mode):
		self.bucket.sythesize(m1,mode)
		
__all__ = ['Bar',"SharedDataManager"]
